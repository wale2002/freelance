...
// import { zodResolver } from '@hookform/resolvers/zod';
// import * as z from 'zod';
// import { generateAIProposal, bidOnProject } from '@/lib/api';
// import { toast } from 'sonner';
// import { Sparkles, Loader2 } from 'lucide-react';
// import {
//   Dialog,
...
//     setGenerating(true);
//     try {
//       const response = await generateAIProposal(projectId);
//       if (response.data.success && response.data.data) {
//         form.setValue('proposal', response.data.data.proposal);
//         toast.success('AI proposal generated!');
//       } else {
//         toast.error(response.data.message || 'Failed to generate proposal');
//       }
//     } catch (error: any) {
...
//   const onSubmit = async (data: z.infer<typeof bidSchema>) => {
//     try {
//       const response = await bidOnProject(projectId, { amount: data.amount, proposal: data.proposal });
//       if (response.data) {
//         toast.success('Bid submitted successfully!');
//         form.reset();
//         onOpenChange(false);
...
interface BidFormProps {
  projectId: string;
  initialProposal?: string; // NEW: Optional prefilled proposal
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}
...
export const BidForm = ({
  ...
  // UPDATED: Prefill proposal when dialog opens with initial value
  useEffect(() => {
    if (open && initialProposal) {
      form.reset({ amount: 0, proposal: initialProposal });
    } else if (open) {
      form.reset(); // Clear on open without prefill
    }
  }, [open, initialProposal, form]);
  ...
  const onSubmit = async (data: z.infer<typeof bidSchema>) => {
    try {
      const response = await bidOnProject(projectId, {
        amount: data.amount,
